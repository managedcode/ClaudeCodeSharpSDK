...
Project: ManagedCode.ClaudeCodeSharpSDK
Stack: .NET 10, C# 14, TUnit, GitHub Actions, NativeAOT, NuGet, Claude Code CLI integration
...
- analyze: `dotnet build ManagedCode.ClaudeCodeSharpSDK.slnx -c Release -warnaserror /p:TreatWarningsAsErrors=true`
...
- For Claude Code CLI metadata checks (for example model list/default), always use the installed `claude` CLI directly first; do not use `cargo`/submodule helper binaries unless explicitly requested.
...
- Always invoke runner options after `--` (for example `dotnet test --solution ManagedCode.ClaudeCodeSharpSDK.slnx -c Release -- --treenode-filter "<pattern>"`); if a focused filter runs zero tests, treat it as an invalid filter and correct it before reporting results.
...
- In this repository, method-level `treenode-filter` patterns resolve at depth 3 (`/*/*/*/<TestMethodName>`). For CLI smoke subset runs use `dotnet test --project ClaudeCodeSharpSDK.Tests/ClaudeCodeSharpSDK.Tests.csproj -c Release --no-build -- --treenode-filter "/*/*/*/ClaudeCli_Smoke_*"`.
- For reliable discovery when selecting focused tests, use the built test app directly: `tests/bin/Release/net10.0/ManagedCode.ClaudeCodeSharpSDK.Tests --list-tests` (the `dotnet test ... -- --list-tests` wrapper can report zero tests in this setup).
...
- For CLI process interaction tests, use the real installed `claude` CLI (no fake CLI process runners in smoke/integration coverage).
- Treat `claude` CLI as a test prerequisite: ensure local/CI test setup installs `claude` before running CLI interaction tests; do not replace this with fakes.
- CI must validate SDK on all Claude-supported desktop/server platforms (macOS, Linux, Windows): run build + tests and include a non-auth smoke check that `claude` is discoverable and invokable.
- CI/release workflow smoke checks are additive gates; they must not replace full `dotnet test --solution ManagedCode.ClaudeCodeSharpSDK.slnx -c Release` execution.
- Never run auth-dependent real Claude tests in CI pipelines; CI must remain non-auth and deterministic.
...
- Do not expose a public SDK type named `Thread`; use `ClaudeThread` to avoid .NET type-name conflicts.
...
- For SDK option/model/flag support, use the real `claude` CLI behavior (`claude --help`, `claude -p --output-format json|stream-json`, runtime checks) as the only source of truth; do not use TypeScript SDK surface as the decision baseline.
- Do not hardcode npm-only Claude install/update guidance; when generating CLI update/install command text, detect and support `bun` where applicable.
- Image/file inputs may exist in the model layer, but docs/tests must accurately reflect current runtime support. Do not claim Claude print-mode image support until it is implemented and validated.
...
- README first examples must be beginner-friendly: avoid advanced/optional knobs (for example `ClaudeExecutablePath`) in the very first snippet.
...
- README must explicitly state runtime prerequisites: `claude` CLI installed and user already authenticated (`claude login`) before SDK usage.
- When a README snippet shows model selection, only document knobs that реально exist in the current SDK surface and verified Claude CLI contract.
...
- Keep API behavior aligned with actual Claude Code CLI contracts first; TypeScript SDK mapping may be used only as an optional historical reference, not as a blocker for C# SDK design.
